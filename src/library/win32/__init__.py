...
from src.library.win32.wintypes import * 
...
_RegisterClass = define_function( user32, 'RegisterClassW', ATOM, (POINTER(WNDCLASS),) )
def RegisterClass(wndClass):
    return _RegisterClass(byref(wndClass))
...
_TranslateMessage = define_function(user32, 'TranslateMessage', BOOL, (POINTER(MSG),) )
def TranslateMessage(msg):
    return _TranslateMessage(byref(msg))

_DispatchMessage = define_function(user32, 'DispatchMessageW', LRESULT, (POINTER(MSG),) )
def DispatchMessage(msg):
    return _DispatchMessage(byref(msg))
...
_PeekMessage = define_function( user32, 'PeekMessageW', BOOL, _peekMessageParams )
def PeekMessage(msg, hwnd, msgFilterMin, msgFilterMax, removeMsg):
    return _PeekMessage(byref(msg), hwnd, msgFilterMin, msgFilterMax, removeMsg)

GetDC = define_function( user32, 'GetDC', HDC, (HWND,) )

_choosePixelFormatParams = (HDC, POINTER(PIXELFORMATDESCRIPTOR_))
_ChoosePixelFormat = define_function( gdi32, 'ChoosePixelFormat', c_int, _choosePixelFormatParams )
def ChoosePixelFormat(devContext, pfd):
    return _ChoosePixelFormat(devContext, pointer(pfd))

_setPixelFormatParams = (HDC, c_int, POINTER(PIXELFORMATDESCRIPTOR_))
_SetPixelFormat = define_function( gdi32, 'SetPixelFormat', BOOL, _setPixelFormatParams )
def SetPixelFormat(devContext, pixelFormat, pfd):
    return _SetPixelFormat(devContext, pixelFormat, pointer(pfd))
...
_GetClientRect = define_function( user32, 'GetClientRect', BOOL, (HWND, LPRECT))
_GetWindowRect = define_function( user32, 'GetWindowRect', BOOL, (HWND, LPRECT))

def GetClientRect(hwnd, rcClient):
    return _GetClientRect(hwnd, byref(rcClient))
    
def GetWindowRect(hwnd, rcWindow):
    return _GetWindowRect(hwnd, byref(rcWindow))
